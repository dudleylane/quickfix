...
void *readOutput(void *p)
{
    ...
    while (true)
    {
        if (!ReadFile(hReadPipe, chBuf, 128, &dwRead, NULL) || dwRead == 0)
        {
            break;
        }
...
void Process::create()
{
    ...
    m_created = CreateProcess(NULL, const_cast<char *>(m_commandLine.c_str()), NULL, NULL, TRUE, getFlags(), NULL, NULL,
                              &info, &m_procinfo) != 0;
...
void Process::openOutFile()
{
    ...
    m_outFileHandle = CreateFile(m_outFile.c_str(), GENERIC_WRITE,
                                 0, // open exclusively
