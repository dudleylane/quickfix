...
struct Exception : public std::logic_error
{
    Exception(const std::string &type, const std::string &detail)
        : std::logic_error(detail.size() ? type + ": " + detail : type), type(type), detail(detail)
    {
    }
    ~Exception() NOEXCEPT {}
...
struct DataDictionaryNotFound : public Exception
{
    DataDictionaryNotFound(const std::string &version, const std::string &what = "")
        : Exception("Could not find data dictionary", what), version(version)
    {
    }
...
struct FieldNotFound : public Exception
{
    FieldNotFound(int field = 0, const std::string &what = "")
        : Exception("Field not found: " + std::to_string(field), what), field(field)
    {
    }
    int field;
};
...
{
    MessageParseError(const std::string &what = "") : Exception("Could not parse message", what) {}
};

/// Not a recognizable message
struct InvalidMessage : public Exception
{
    InvalidMessage(const std::string &what = "") : Exception("Invalid message", what) {}
};

/// %Application is not configured correctly
struct ConfigError : public Exception
{
    ConfigError(const std::string &what = "") : Exception("Configuration failed", what) {}
};
...
struct InvalidTagNumber : public Exception
{
    InvalidTagNumber(int field = 0, const std::string &what = "")
        : Exception("Invalid tag number: " + std::to_string(field), what), field(field)
    {
    }
    int field;
};
...
struct RequiredTagMissing : public Exception
{
    RequiredTagMissing(int field = 0, const std::string &what = "")
        : Exception("Required tag missing: " + std::to_string(field), what), field(field)
    {
    }
    int field;
};
...
struct TagNotDefinedForMessage : public Exception
{
    TagNotDefinedForMessage(int field = 0, const std::string &what = "")
        : Exception("Tag not defined for this message type: " + std::to_string(field), what), field(field)
    {
    }
    int field;
};
...
struct NoTagValue : public Exception
{
    NoTagValue(int field = 0, const std::string &what = "")
        : Exception("Tag specified without a value: " + std::to_string(field), what), field(field)
    {
    }
    int field;
};
...
struct IncorrectTagValue : public Exception
{
    IncorrectTagValue(int field = 0, const std::string &what = "")
        : Exception("Value is incorrect (out of range) for this tag: " + std::to_string(field), what), field(field)
    {
    }
    int field;
};
...
struct IncorrectDataFormat : public Exception
{
    IncorrectDataFormat(int field = 0, const std::string &what = "")
        : Exception("Incorrect data format for value with tag: " + std::to_string(field), what), field(field)
    {
    }
    int field;
};
...
{
    IncorrectMessageStructure(const std::string &what = "") : Exception("Incorrect message structure", what) {}
};

/// Field shows up twice in the message
struct DuplicateFieldNumber : public Exception
{
    DuplicateFieldNumber(const std::string &what = "") : Exception("Duplicate field number", what) {}
};

/// Not a known message type
struct InvalidMessageType : public Exception
{
    InvalidMessageType(const std::string &what = "") : Exception("Invalid Message Type", what) {}
};

/// Message type not supported by application
struct UnsupportedMessageType : public Exception
{
    UnsupportedMessageType(const std::string &what = "") : Exception("Unsupported Message Type", what) {}
};

/// Version of %FIX is not supported
struct UnsupportedVersion : public Exception
{
    UnsupportedVersion(const std::string &what = "") : Exception("Unsupported Version", what) {}
};
...
struct TagOutOfOrder : public Exception
{
    TagOutOfOrder(int field = 0, const std::string &what = "")
        : Exception("Tag specified out of required order", what), field(field)
    {
    }
    int field;
};
...
struct RepeatedTag : public Exception
{
    RepeatedTag(int field = 0, const std::string &what = "")
        : Exception("Repeated tag not part of repeating group", what), field(field)
    {
    }
    int field;
};
...
struct RepeatingGroupCountMismatch : public Exception
{
    RepeatingGroupCountMismatch(int field = 0, const std::string &what = "")
        : Exception("Repeating group count mismatch", what), field(field)
    {
    }
    int field;
};
...
{
    DoNotSend(const std::string &what = "") : Exception("Do Not Send Message", what) {}
};

/// User wants to reject permission to logon
struct RejectLogon : public Exception
{
    RejectLogon(const std::string &what = "") : Exception("Rejected Logon Attempt", what) {}
};

/// Session cannot be found for specified action
struct SessionNotFound : public Exception
{
    SessionNotFound(const std::string &what = "") : Exception("Session Not Found", what) {}
};
...
{
    SocketException() : Exception("Socket Error", errorToWhat()) {}

    SocketException(const std::string &what) : Exception("Socket Error", what) {}
