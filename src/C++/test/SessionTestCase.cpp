...
{
    FIX42::NewOrderSingle newOrderSingle(ClOrdID("ID"), HandlInst('1'), Symbol("SYMBOL"), Side(Side_BUY),
                                         TransactTime::now(), OrdType(OrdType_MARKET));
...
{
    FIX42::ExecutionReport executionReport(OrderID("ID"), ExecID("ID"), ExecTransType('0'), ExecType('0'),
                                           OrdStatus('0'), Symbol("SYMBOL"), Side(Side_BUY), LeavesQty(100), CumQty(0),
                                           AvgPx(0));
...
{
    FIX40::ExecutionReport executionReport(OrderID("ID"), ExecID("123"), ExecTransType('0'), OrdStatus('0'),
                                           Symbol("SYMBOL"), Side(Side_BUY), OrderQty(100), LastShares(0), LastPx(0),
                                           CumQty(0), AvgPx(0));
    fillHeader(executionReport.getHeader(), sender, target, seq);
...
{
    FIX50::ExecutionReport executionReport(OrderID("ID"), ExecID("ID"), ExecType('0'), OrdStatus('0'), Side(Side_BUY),
...
public:
    TestCallback()
        : now(UtcTimeStamp::now()), startTimeStamp(UtcTimeStamp::now()), endTimeStamp(UtcTimeStamp::now()),
          endTime(startTime)
    {
    }
...
struct ExceptionDataDictionary : public DataDictionary
{
    ...
    ExceptionDataDictionary(DataDictionary &dictionary, ExceptionType type)
        : DataDictionary(dictionary), exceptionType(type) {};
...
TEST_CASE_METHOD(sessionFixture, "SessionTestCase")
{
    ...
    {
        DataDictionaryProvider provider;
        provider.addTransportDataDictionary(BeginString("FIX.4.2"),
                                            std::shared_ptr<DataDictionary>(new DataDictionary()));
        auto const timestamper = [this]() { return now; };

        auto createSession = [&](SenderCompID senderCompID, TargetCompID targetCompID)
        {
            return new Session(timestamper, *this, factory,
                               SessionID(BeginString("FIX.4.2"), senderCompID, targetCompID), provider,
    ...
    SECTION("lookupSession")
    {
        ...
        auto createSession = [&](SenderCompID senderCompID, TargetCompID targetCompID)
        {
            return Session(timestamper, *this, factory, SessionID(BeginString("FIX.4.2"), senderCompID, targetCompID),
                           provider, TimeRange(UtcTimeOnly(), UtcTimeOnly()), 0, 0);
        };
    ...
    SECTION("registerSession")
    {
        ...
        Session *pSession = new Session([this]() { return now; }, *this, factory,
        ...
        CHECK(
            Session::isSessionRegistered(SessionID(BeginString("FIX.4.2"), SenderCompID("TW"), TargetCompID("ISLD"))));
        CHECK(nullptr ==
              Session::registerSession(SessionID(BeginString("FIX.4.2"), SenderCompID("TW"), TargetCompID("ISLD"))));
    ...
    SECTION("sessionCapableOfLogging")
    {
        ...
        providerCustom.addTransportDataDictionary(sessionIDCustom.getBeginString(),
    ...
    SECTION("next_IOExceptionHandledAndNotLoggedOn")
    {
        ...
        Session sessionObject = Session([this]() { return now; }, *this, exceptionFactory, sessionIDCustom,
                                        providerCustom, sessionTimeCustom, 1, 0);
...
TEST_CASE_METHOD(initiatorFixture, "InitiatorSessionTestCase")
{
    ...
    SECTION("invalidRawFixMessage_LogonMessageParsingError_InvalidMessageException")
    {
        ...
        newOrderSingleRawFIX = "8=FIX.4.2" + delimSOH + "9=114" + delimSOH // missing Message type in header (35=A)
                               + "34=2" + delimSOH + "49=ISLD" + delimSOH + "56=TW" + delimSOH +
                               "52=20190517-16:08:43" + delimSOH + "98=0" + delimSOH + "108=1" + delimSOH + "10=166" +
                               delimSOH;
...
TEST_CASE_METHOD(acceptorFixture, "AcceptorSessionTestCase")
{
    ...
    SECTION("gapFillDoesNotRetainSkippedMessages")
    {
...
TEST_CASE_METHOD(acceptorT11Fixture, "AcceptorT11TestCase")
{
    ...
    SECTION("sendNextExpectedMsgSeqNum")
    {
        ...
        CHECK(0 ==
              toResendRequest); // no resend request for SeqNum 5-9, initiator is expected to start message recovery
...
struct initiatorCreatedBeforeStartTimeFixture : public TestCallback
{
    ...
    initiatorCreatedBeforeStartTimeFixture() : actuallySent(0), actuallySentLogon(false)
    {
        now.setCurrent();
        startTime = now;
        startTime.setMillisecond(0);
        ...
        DataDictionaryProvider provider;
        provider.addTransportDataDictionary(sessionID.getBeginString(), FIX::TestSettings::pathForSpec("FIX42"));
        object = new Session([this]() { return now; }, *this, messageStoreFactory, sessionID, provider, sessionTime,
                             HEARTBTINT, 0);
    }
