...
TEST_CASE("SocketConnectionTests")
{
    ...
    struct TestSession : public Session
    {
        TestSession(Application &app, MessageStoreFactory &factory, const SessionID &sessionId,
            ...
            : Session([]() { return UtcTimeStamp::now(); }, app, factory, sessionId, provider, timeRange, heartBtInt,
                      nullptr) {};
        ~TestSession() {};
    ...
    {
        TestSocketInitiator(Application &app, MessageStoreFactory &factory, const SessionSettings &settings,
                            Session *session)
    ...
    {
        TestSocketAcceptor(Application &app, MessageStoreFactory &factory, const SessionSettings &settings,
                           Session *session, LogFactory &log)
    ...
    struct BaseSocketConnection
    {
        BaseSocketConnection()
        {
            ...
            initiatorProvider.addTransportDataDictionary(initiatorSessionID.getBeginString(),
                                                         FIX::TestSettings::pathForSpec("FIX42"));
            acceptorProvider.addTransportDataDictionary(acceptorSessionID.getBeginString(),
                                                        FIX::TestSettings::pathForSpec("FIX42"));
