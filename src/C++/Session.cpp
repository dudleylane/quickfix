...
Session::Session(std::move_only_function<UtcTimeStamp()> timestamper, Application &application,
    ...
    : m_timestamper(std::move(timestamper)), m_application(application), m_sessionID(sessionID),
      m_sessionTime(sessionTime), m_logonTime(sessionTime), m_senderDefaultApplVerID(ApplVerID_FIX50),
      m_targetDefaultApplVerID(ApplVerID_FIX50), m_sendRedundantResendRequests(false), m_checkCompId(true),
      m_checkLatency(true), m_maxLatency(120), m_resetOnLogon(false), m_resetOnLogout(false),
      m_resetOnDisconnect(false), m_refreshOnLogon(false), m_timestampPrecision(3), m_persistMessages(true),
      m_validateLengthAndChecksum(true), m_sendNextExpectedMsgSeqNum(false), m_isNonStopSession(false),
      m_state(m_timestamper()), m_dataDictionaryProvider(dataDictionaryProvider),
      m_messageStoreFactory(messageStoreFactory), m_pLogFactory(pLogFactory), m_pResponder(0)
{
...
void Session::nextLogon(const Message &logon, const UtcTimeStamp &now)
{
    ...
    if (isTargetTooHigh(msgSeqNum) && !resetSeqNumFlag)
    {
        ...
        {
            m_state.onEvent("Expecting retransmits FROM: " + SEQNUM_CONVERTOR::convert(getExpectedTargetNum()) +
...
void Session::nextSequenceReset(const Message &sequenceReset, const UtcTimeStamp &now)
{
    ...
    {
        m_state.onEvent("Received SequenceReset FROM: " + SEQNUM_CONVERTOR::convert(getExpectedTargetNum()) +
    ...
    for (i = messages.begin(); i != messages.end(); ++i)
    {
        ...
        if (m_sessionID.isFIXT())
        {
            ...
            else
            {
                ...
                pMsg.reset(new Message(headerOrder, trailerOrder, messageOrder, *i, sessionDD, applicationDD,
        ...
        else
        {
            ...
            else
            {
                ...
                pMsg.reset(
                    new Message(headerOrder, trailerOrder, messageOrder, *i, sessionDD, m_validateLengthAndChecksum));
            }
        }
...
void Session::generateLogon(const Message &aLogon)
{
    ...
    {
        logon.setField(NextExpectedMsgSeqNum(getExpectedTargetNum() +
                                             1)); // +1 because incoming Logon did not increment the target SeqNum yet
    }
    fill(logon.getHeader());
    sendRaw(logon);
    m_state.sentLogon(true);
...
void Session::generateResendRequest(const BeginString &beginString, const MsgSeqNum &msgSeqNum)
{
    ...
    m_state.onEvent("Sent ResendRequest FROM: " + SEQNUM_CONVERTOR::convert(beginSeqNo) +
                    " TO: " + SEQNUM_CONVERTOR::convert(endSeqNo));
...
void Session::generateReject(const Message &message, int err, int field)
{
    ...
    if (reason && (field || err == SessionRejectReason_INVALID_TAG_NUMBER))
    {
        ...
        m_state.onEvent("Message " + msgSeqNum.getString() + " Rejected: " + reason + ":" +
                        SEQNUM_CONVERTOR::convert(field));
    }
    else if (reason)
    {
...
void Session::doTargetTooHigh(const Message &msg)
{
    ...
    if (m_state.resendRequested())
    {
        ...
        {
            m_state.onEvent("Already sent ResendRequest FROM: " + SEQNUM_CONVERTOR::convert(range.first) +
...
bool Session::sendToTarget(Message &message, const SenderCompID &senderCompID, const TargetCompID &targetCompID,
...
bool Session::sendToTarget(Message &message, const std::string &sender, const std::string &target,
                           const std::string &qualifier) EXCEPT(SessionNotFound)
{
