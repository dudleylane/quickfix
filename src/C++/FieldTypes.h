...
public:
    ...
    UtcTimeStamp(DateTime dateTime) : DateTime(std::move(dateTime)) {}

    /// Defaults to the current date
    UtcTimeStamp(int hour, int minute, int second, int millisecond = 0) : DateTime(DateTime::nowUtc())
    {
        setHMS(hour, minute, second, millisecond);
    }

    UtcTimeStamp(int hour, int minute, int second, int fraction, int precision) : DateTime(DateTime::nowUtc())
    {
    ...
    explicit UtcTimeStamp(time_t time, int millisecond = 0) : DateTime(fromUtcTimeT(time, millisecond)) {}

    UtcTimeStamp(time_t time, int fraction, int precision) : DateTime(fromUtcTimeT(time, fraction, precision)) {}

    UtcTimeStamp(const tm *time, int millisecond = 0) : DateTime(fromTm(*time, millisecond)) {}
...
public:
    ...
    /// Defaults to the current date
    LocalTimeStamp(int hour, int minute, int second, int millisecond = 0) : DateTime(DateTime::nowLocal())
    {
        setHMS(hour, minute, second, millisecond);
    }
    ...
    explicit LocalTimeStamp(time_t time, int millisecond = 0) : DateTime(fromLocalTimeT(time, millisecond)) {}

    LocalTimeStamp(time_t time, int fraction, int precision) : DateTime(fromLocalTimeT(time, fraction, precision)) {}

    LocalTimeStamp(const tm *time, int millisecond = 0) : DateTime(fromTm(*time, millisecond)) {}
...
public:
    ...
    UtcTimeOnly(const DateTime &val) : DateTime(val) { clearDate(); }
    ...
    explicit UtcTimeOnly(time_t time, int millisecond = 0) : DateTime(fromUtcTimeT(time, millisecond)) { clearDate(); }

    UtcTimeOnly(time_t time, int fraction, int precision) : DateTime(fromUtcTimeT(time, fraction, precision))
    {
        clearDate();
    }

    UtcTimeOnly(const tm *time, int millisecond = 0) : DateTime(fromTm(*time, millisecond)) { clearDate(); }

    UtcTimeOnly(const tm *time, int fraction, int precision) : DateTime(fromTm(*time, fraction, precision))
    {
        clearDate();
    }
...
public:
    ...
    LocalTimeOnly(const DateTime &val) : DateTime(val) { clearDate(); }
    ...
    explicit LocalTimeOnly(time_t time, int millisecond = 0) : DateTime(fromLocalTimeT(time, millisecond))
    {
        clearDate();
    }

    LocalTimeOnly(time_t time, int fraction, int precision) : DateTime(fromLocalTimeT(time, fraction, precision))
    {
        clearDate();
    }

    LocalTimeOnly(const tm *time, int millisecond = 0) : DateTime(fromTm(*time, millisecond)) { clearDate(); }

    LocalTimeOnly(const tm *time, int fraction, int precision) : DateTime(fromTm(*time, fraction, precision))
    {
        clearDate();
    }
...
public:
    ...
    UtcDate(const DateTime &val) : DateTime(val) { clearTime(); }

    UtcDate(int date, int month, int year) : DateTime(year, month, date, 0, 0, 0, 0) {}

    UtcDate(int sec) : DateTime(sec / DateTime::SECONDS_PER_DAY, 0) {}

    UtcDate(const tm *time) : DateTime(fromTm(*time)) { clearTime(); }
...
public:
    ...
    LocalDate(const DateTime &val) : DateTime(val) { clearTime(); }

    LocalDate(int date, int month, int year) : DateTime(year, month, date, 0, 0, 0, 0) {}

    LocalDate(int sec) : DateTime(sec / DateTime::SECONDS_PER_DAY, 0) {}

    LocalDate(const tm *time) : DateTime(fromTm(*time)) { clearTime(); }
