...
public:
    SessionFactory(Application &application, MessageStoreFactory &messageStoreFactory, LogFactory *pLogFactory)
        : m_application(application), m_messageStoreFactory(messageStoreFactory), m_pLogFactory(pLogFactory)
    {
    }
...
private:
    ...
    void processFixtDataDictionaries(const SessionID &sessionID, const Dictionary &settings,
                                     DataDictionaryProvider &provider) EXCEPT(ConfigError);

    void processFixDataDictionary(const SessionID &sessionID, const Dictionary &settings,
                                  DataDictionaryProvider &provider) EXCEPT(ConfigError);
